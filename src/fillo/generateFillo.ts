...
const dfs = (r: number, c: number, gridSize: number, right: Array<Array<boolean>>, down: Array<Array<boolean>>, visited: Array<boolean>) : number => {
    ...
    let total = 1
    if (r > 0 && down[r - 1][c]) total += dfs(r-1, c, gridSize, right, down, visited)
    if (c > 0 && right[r][c - 1]) total += dfs(r, c - 1, gridSize, right, down, visited)
    if (down[r][c]) total += dfs(r+1, c, gridSize, right, down, visited)
    if (right[r][c]) total += dfs(r, c+1, gridSize, right, down, visited)
    return total
}

const generateArray = (rand: RandomSeed, gridSize: number, p: number) : Array<Array<boolean>> => {
    return new Array(gridSize).fill(false).map(() => new Array<boolean>(gridSize).fill(false).map(() => (rand.random() < p)))
}
